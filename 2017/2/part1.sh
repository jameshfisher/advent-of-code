...
declare -i checksum cell i
declare -a row
declare min max  # not integers because they hold '' as initial null value
while read -a row; do
  min=
  max=
  for ((i = 0; i < ${#row[@]}; i++)); do
    cell="${row[i]}"
    if [ "$min" = "" ] || [ "$cell" -lt "$min" ] ; then
      let 'min = cell'
    elif [ "$max" = "" ] || [ "$max" -lt "$cell" ] ; then
      let 'max = cell'
    fi
  done
  let 'checksum += (max - min)'
done < input.txt
echo $checksum
