...
declare -i checksum rowlen cell1 cell2 i j
declare -a row
while read -a row; do
  rowlen="${#row[@]}"
  for ((i = 0; i < rowlen; i++)); do
    cell1="${row[i]}"
    for ((j = 0; j < rowlen; j++)); do
      cell2="${row[j]}"
      if [ "$cell1" -lt "$cell2" ] && [ "$((($cell2 / $cell1) * $cell1))" -eq "$cell2" ]; then
        let 'checksum += cell2/cell1'
        continue 3
      fi
    done
  done
  echo "Failed to find the pair!"
  exit
done < input.txt
echo $checksum
